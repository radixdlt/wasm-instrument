...
pub trait Translator {
    ...
    fn translate_type_def(&self, ty: Type, s: &mut TypeSection) -> Result<()> {
    ...
    fn translate_table_type(&self, ty: &wasmparser::TableType) -> Result<wasm_encoder::TableType> {
    ...
    fn translate_tag_type(&self, ty: &wasmparser::TagType) -> Result<wasm_encoder::TagType> {
    ...
    fn translate_element(&self, e: wasmparser::Element<'_>, s: &mut ElementSection) -> Result<()> {
        element(self.as_obj(), e, s)
    }

    fn translate_data(&self, d: wasmparser::Data<'_>, s: &mut DataSection) -> Result<()> {
        data(self.as_obj(), d, s)
    }

    fn translate_code(&self, body: FunctionBody<'_>, s: &mut CodeSection) -> Result<()> {
        code(self.as_obj(), body, s)
    }

    fn translate_op(&self, e: &Operator<'_>) -> Result<Instruction<'static>> {
        op(self.as_obj(), e)
    }

    fn translate_block_type(&self, ty: &wasmparser::BlockType) -> Result<BlockType> {
        block_type(self.as_obj(), ty)
    }

    fn translate_memarg(&self, arg: &wasmparser::MemArg) -> Result<MemArg> {
...
pub fn type_def(t: &dyn Translator, ty: Type, s: &mut TypeSection) -> Result<()> {
    match ty {
        Type::Func(f) => {
            s.function(
                f.params()
...
pub fn tag_type(_: &dyn Translator, ty: &wasmparser::TagType) -> Result<wasm_encoder::TagType> {
...
pub fn op(t: &dyn Translator, op: &Operator<'_>) -> Result<Instruction<'static>> {
    ...
    Ok(match op {
        ...
        O::DataDrop { segment } => I::DataDrop(*segment),
        ...
        O::ElemDrop { segment } => I::ElemDrop { segment: *segment },
        O::TableCopy {
            dst_table,
            src_table,
        } => I::TableCopy {
        ...
        O::TableFill { table } => I::TableFill { table: *table },
        O::TableGet { table } => I::TableGet { table: *table },
        O::TableSet { table } => I::TableSet { table: *table },
        O::TableGrow { table } => I::TableGrow { table: *table },
        O::TableSize { table } => I::TableSize { table: *table },
        ...
        | O::AtomicFence { .. } => return Err(anyhow!("no_mutations_applicable")),
    })
}
...
pub fn memarg(_: &dyn Translator, memarg: &wasmparser::MemArg) -> Result<MemArg> {
    Ok(MemArg {
...
#[allow(unused)]
pub fn data(t: &dyn Translator, data: wasmparser::Data<'_>, s: &mut DataSection) -> Result<()> {
    let offset;
...
pub fn code(t: &dyn Translator, body: FunctionBody<'_>, s: &mut CodeSection) -> Result<()> {
    let locals = body
        .get_locals_reader()?
        .into_iter()
        .map(|local| {
